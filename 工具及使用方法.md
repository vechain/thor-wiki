...
|字段 | 说明 | 备注 |
| --- | --- | --- |
| contract Owner | 部署合约人 | 将从account列表中选取 |
...
### 方法2 : Java Client SDK
1. API方式:
详见[Transaction client](https://github.com/vechain/thor-client-sdk4j#transactionclient)
...
---
### 方法2 : Java Client SDK
详见[TransactionClientTest.java](https://github.com/vechain/thor-client-sdk4j/blob/master/src/main/java/com/vechain/thorclient/clients/TransactionClient.java)

## MPP相关
详见[Thor-wiki](https://github.com/vechain/thor-wiki)
## 链上信息查询
...
#### 最新块/区块查询/区块ID
默认进入insights会实时加载最新块查询，若需要查询特定区块编号信息，可通过输入区块查询
...
#### 交易查询
由于Insights为已链上数据查询， 查询到的交易皆有Receipt
>若有Branch标签，则代表此区块并非在主链上
...
### 方法2: Java Client SDK
API:
详见[thorClient](https://github.com/vechain/thor-client-sdk4j/tree/master/src/main/java/com/vechain/thorclient/clients)
...
或
2.java api:
Block block = BlockClient.getBlock(Revision.BEST);  //获得最新的块 get best block
    或 Block block = BlockClient.getBlock(148847);  //获取指定的块 get specified block
...
#### 4. 判断交易是否有效
- 建议交易确认区块为 `12` 个区块以上
- 在区块中的交易说明都被执行了。但交易是否执行成功，可以查看交易单据的`reverted`字段，如果reverted字段为false，则说明交易被成功执行了。如果reverted字段为true，说明交易执行失败。

- 在交易Receipt中， Output将会显示此交易所包含的转账信息
    ###### 1. VTHO:
