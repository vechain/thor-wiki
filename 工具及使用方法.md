...
| to  | 交易接收地址 |-  |
| value | 转账金额 | VET为单位 |
| expiration | 用于设置交易过期/失效时间 | 区块为单位/建议值为18<br> Block Number ∈[blockRef.number , BlockRef.number + expiration] |
...
| expiration | 用于设置交易过期/失效 | 区块为单位/建议值为18<br> Block Number ∈[blockRef.number , BlockRef.number + expiration]  |
